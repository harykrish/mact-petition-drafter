# Cambrian — 1-Minute Demo Script

## Narration (~60 seconds)

A catastrophic road accident doesn't just create a legal problem. It creates a medical crisis, a financial crisis, and a communication crisis — all at once. The family is buried in hospital records, police reports, insurance papers. Over a million of these cases are stuck in Indian tribunals right now. Ten billion dollars unpaid.

Cambrian builds a living knowledge base from all of it. Every fact sourced, every conflict flagged, every update tracked. From that one KB, multiple agents act — legal drafting, medical advocacy, rehab planning, even AAC communication for patients who've lost the ability to speak.

Today, we're showing the legal agent. Watch.

Nine documents go in. Claude extracts structured facts, and a reconciler catches conflicts across sources. The police report says March fifth, the hospital says March sixth — that contradiction gets flagged, never buried.

An independent verifier — fresh context, no memory of the extraction — audits the knowledge base against ten invariants. Only when every check passes does the system commit.

Claude drafts the petition. A second independent agent re-derives every figure from scratch. Over a million dollars in lost earnings. If a single number is wrong, it rejects and loops back — autonomously.

One knowledge base. Twelve orchestrated Claude calls. Two independent verification agents. From weeks to minutes.

## Screen Recording Guide

| Time | Screen | Narration cue |
|------|--------|---------------|
| 0:00-0:06 | Intro slide | "A catastrophic road accident..." |
| 0:06-0:13 | Home page — architecture diagram | "Cambrian builds a living..." |
| 0:13-0:18 | Home page — workflow tiles | "...legal drafting, medical advocacy..." |
| 0:18-0:28 | Click Replay — documents ingest | "Nine documents go in..." |
| 0:28-0:34 | Contradiction panel pulses | "The police report says March fifth..." |
| 0:34-0:40 | KB verify PASS | "An independent verifier..." |
| 0:40-0:50 | Petition tab — stats + verifier | "Claude drafts the petition..." |
| 0:50-0:55 | Arithmetic match section | "Over a million dollars..." |
| 0:55-1:00 | Stat strip close-up | "One knowledge base..." |

## Sources
- [10.73 lakh pending MACT cases, Rs 96,257 crore](https://www.legacyias.com/motor-accident-compensation-crisis-mact/)
- [90% claims stalled by documentation issues](https://m.thewire.in/article/government/for-indias-road-accident-survivors-legal-compensation-is-elusive)
- [Rs 80,000 crore unpaid compensation](https://indialegallive.com/column-news/road-accident-victms-india-highest-fatalities-in-world-rs-80000-crore-compensation-unpaid/)
- [155,000+ annual road deaths](https://www.pib.gov.in/PressReleasePage.aspx?PRID=2036268)
